...
# =============================================================================
# 3. Training Sign-Off Report
# =============================================================================
def generate_training_signoff_pdf(signoff):
    """
    Generate a Training Sign-Off PDF.
    """
    buffer = BytesIO()
    doc, drawable_width = _new_doc(buffer)
    styles = _get_styles()
    ...
    training_data = [
        [Paragraph("<b>Contractor:</b>", styles["Label"]),
         Paragraph(get_val(signoff.contractor.contractor_name), styles["Value"]),
        ...
        [Paragraph("<b>Number of Workers:</b>", styles["Label"]),
         ...
         Paragraph("<b>Sign-Off Date:</b>", styles["Label"]),
    ...
    company_data = [
        [Paragraph("<b>Declaration:</b>", styles["Label"]),
         Paragraph("I confirm that the above listed contractor and its workers have received the required safety training.", styles["Value"])],
        [Paragraph("<b>Agreed:</b>", styles["Label"]),
         Paragraph("Yes" if signoff.company_declaration else "No", styles["Value"])],
        [Paragraph("<b>Company Representative:</b>", styles["Label"]),
    ...
    # Company Signature
    if signoff.company_signature:
        sig_path = _safe_file_path(signoff.company_signature)
        if sig_path:
            story.append(Spacer(1, 4 * mm))
            story.append(Paragraph("<b>Company Representative Signature:</b>", styles["Label"]))
            _embed_image(story, sig_path, styles["Value"], max_width_mm=60, max_height_mm=30)
    ...
    contractor_data = [
        [Paragraph("<b>Declaration:</b>", styles["Label"]),
         Paragraph("I confirm that the above listed workers have received the required safety training.", styles["Value"])],
        [Paragraph("<b>Agreed:</b>", styles["Label"]),
         Paragraph("Yes" if signoff.contractor_declaration else "No", styles["Value"])],
    ...
    contractor_table = Table(contractor_data, colWidths=[col4, col4 * 3])
    ...
    if signoff.supporting_documents:
        ...
        if doc_path:
            story.append(Spacer(1, 4 * mm))
            story.append(Paragraph("<b>Supporting Document:</b>", styles["Label"]))
            _embed_image(story, doc_path, styles["Value"], max_width_mm=80, max_height_mm=60)
...
def generate_inspection_pdf(inspection):
    ...
    score_data = [
        ...
        [Paragraph("<b>NO:</b>", styles["Label"]),
         ...
         Paragraph("<b>N/A:</b>", styles["Label"]),
         Paragraph(str(na_count), styles["Value"])],
...
# =============================================================================
# 5. Contractor Log Book (per-year register)
# =============================================================================
