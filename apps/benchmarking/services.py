...
def refresh_live_results_for_all():
    """Refresh current, unpublished periods for every active plant."""
    refresh_live_results_for_plants(Plant.objects.filter(is_active=True))


def refresh_live_results_for_plants(plants):
    """Recalculate current, unpublished periods for the supplied plants."""
...
def calculate_period(period, plants=None):
    ...
    period = BenchmarkPeriod.objects.select_for_update().get(pk=period.pk)
    if period.status == period.Status.CALCULATING:
        return False
    ...
    try:
        ...
        for plant in plant_queryset:
            ...
            for category in categories:
                ...
                for kpi in category.kpis.filter(is_active=True):
                    ...
                    if score is not None:
                        kpi_scores.append((score, kpi.weightage))
                    if target is not None:
                        kpi_targets.append((target, kpi.weightage))
                # Do not convert unavailable source data into a zero score.
                # Only categories with actual calculated KPI values participate.
                if kpi_scores:
                    category_scores.append((calculate_category_score(kpi_scores), category.weightage))
                if kpi_targets:
                    category_targets.append((calculate_overall_target(kpi_targets), category.weightage))
            overall = calculate_overall_score(category_scores)
            overall_target = calculate_overall_target(category_targets)
            ...
            result.overall_score = overall; result.target_score = overall_target; result.target_gap = overall - overall_target if overall_target is not None else None; result.previous_score = previous.overall_score if previous else None; result.previous_gap = overall - previous.overall_score if previous else None
            result.trend = calculate_trend(overall, result.previous_score, TREND_THRESHOLD)
...
def _gaps_and_insights(result):
    below_target_items = list(result.kpi_results.select_related("kpi", "kpi__category").filter(score__lt=100))
    below_target_kpi_ids = [item.kpi_id for item in below_target_items]
    result.gaps.filter(gap_type="KPI").exclude(kpi_id__in=below_target_kpi_ids).delete()
    open_insights = result.insights.filter(status="OPEN", related_kpi__isnull=False)
    open_insights.exclude(related_kpi_id__in=below_target_kpi_ids).delete()
    for item in below_target_items:
        priority = "HIGH" if item.score < 60 else "MEDIUM" if item.score < 80 else "LOW"
        BenchmarkGap.objects.update_or_create(
            benchmark_result=result, kpi=item.kpi, gap_type="KPI",
            defaults={"category": item.kpi.category, "actual_value": item.raw_value or 0, "reference_value": item.target_value or 0, "gap_value": item.gap or 0, "priority": priority, "reason": "KPI is below its configured target."},
        )
        if priority == "HIGH":
            insight, created = BenchmarkInsight.objects.get_or_create(
                benchmark_result=result, related_kpi=item.kpi,
                defaults={"title": f"Improve {item.kpi.name}", "description": "This KPI is materially below target.", "reason": "Calculated score below 60.", "priority": priority, "recommendation": "Review source records, assign actions, and track closure.", "related_category": item.kpi.category},
            )
            if not created:
                BenchmarkInsight.objects.filter(pk=insight.pk).update(
                    title=f"Improve {item.kpi.name}", description="This KPI is materially below target.", reason="Calculated score below 60.", priority=priority, recommendation="Review source records, assign actions, and track closure.", related_category=item.kpi.category,
                )
